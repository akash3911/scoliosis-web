...
const ExportPopover: React.FC<IExportPopoverProps> = ({}) => {
  ...
  async function handleExportPdf() {
    ...
    if (popup) {
      popup.document.title = "Scoliotect PDF";
      popup.document.body.style.fontFamily = "sans-serif";
      popup.document.body.style.padding = "16px";
      popup.document.body.innerText = "Generating PDF...";
    }

    const snapshot = refetchCanvasURL();
    if (!snapshot) {
    ...
    try {
      const blob = await pdf(
        <ScoliotectDocument
          imageSrc={snapshot}
          originalImageSrc={selectedFile?.src}
      ...
      // Revoke later to avoid breaking newly opened tab.
